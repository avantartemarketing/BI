...
| **Funnel by channel**, waterfall view | the same opening - target, stretch, benchmark - above the per-stage rows, every reference read off the basket's pace by today (sessions, implied sends, the benchmark budget) so the rows sum to actual − benchmark, off the same snapshot figures. The channel names stand to the left of their rows rather than on rows of their own, and grey 1px drops carry the running level from each row to the next as on the outcome waterfall. The tall card prints no figure column: a row's figures are in its popup, on the bar or on its name, and the levels print theirs beside the label. Pointing at a name lights its bar and a bar its name; a channel's name lights all of its rows | - |
| **Funnel by channel**, 2 × 2 | the waterfall view with two columns by two rows of room: the bars run across the card on a unit axis (gridlines behind the rows, the figures at the foot) and the figures keep a column of their own. Not on the page by default; added from the layout editor | - |
