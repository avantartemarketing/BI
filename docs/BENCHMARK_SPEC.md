...
| **Funnel by channel**, 2 × 2 | the waterfall view's rows with each stage's rung beside its step, on one label column - the marks of both views unchanged; the anchors (target, stretch, benchmark, actual) carry no rung, and the rung's centre line runs through the group rows only. Not on the page by default; added from the layout editor | - |
